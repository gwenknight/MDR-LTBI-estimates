...
###********** Home ************************************************************************************************#######
home <- "~/Documents/MDR-LTBI-paper/"
setwd(home)

###********** Load code and data ************************************************************************************************#######
source("code_final/cohort_ltbi_mdr.R") # loads function for underyling cohort model

## Population size 2014
load('data_final/POP2014.Rdata')  

## WHO data
## NOT SUPPLIED WITH REPOSTIORT
#w_data <- read.csv("data_final/new_who_edited_sub.csv")[,-1]

## Which countries? 
cni <- read.csv("data_final/138_final_list_included_countries.csv",stringsAsFactors = FALSE)[,-1]

# READ IN
load("data_final/all0_p_ds_mdr.Rdata")
...
for(cci in 1:llu){
  ...
  #save_plot(paste0("output_final/",cni[cci],"_mdr_trends_with_data.pdf"), a1, base_aspect_ratio = 2 )
  
  a2 <- ggplot(rdata, aes(x=year, y = mdr_ari, group = factor(replicate))) + geom_line() + 
    scale_y_continuous("MDR ARI") + scale_x_continuous("Year") 
  save_plot(paste0("output_final/",cni[cci],"_mdr_ari.pdf"), a2, base_aspect_ratio = 2 )
  ...
  write.csv(sa, paste0("output_final/",cni[cci],"_sa_",nari,".csv"))
  
  # Just recent infection 
  w<-which(sa$year > 2012)
  write.csv(sa[w,], paste0("output_final/",cni[cci],"_rec_infec_",nari,".csv"))
...
# Store
write.csv(level2014, paste0("output_final/level2014_",nari,".csv"))

s_level0 <- s_level
s_level$pop_name <- cni[as.numeric(s_level0$popf)]
s_level <- as.data.table(s_level)
# Store
write.csv(s_level, paste0("output_final/s_level_",nari,".csv"))

### OR READ IN
#level2014 <- read.csv("output_final/level2014_10_lin.csv",stringsAsFactors = FALSE)[,-1]
#s_level <- read.csv("output_final/s_level_10_lin.csv", stringsAsFactors = FALSE)[,-1]
...
save_plot(paste0("output_final/ltbi_all_countries_r",nari,".pdf"), a2r, base_aspect_ratio = 1.5 )
...
save_plot(paste0("output_final/ltbi_all_countries_s",nari,".pdf"), a2s, base_aspect_ratio = 1.5 )
...
save_plot(paste0("output_final/ltbi_all_countries_r_mean",nari,".pdf"), a2r, base_aspect_ratio = 0.8)
...
save_plot(paste0("output_final/ltbi_all_countries_s_mean",nari,".pdf"), a2s, base_aspect_ratio = 1.5 )
...
pdf(paste0("output_final/map_ltbis_",nari,".pdf"))
mapParams <- mapCountryData(mapped_data, nameColumnToPlot = "ltbis", catMethod = seq(0,50,10),
                            colourPalette = cols,
                            addLegend = FALSE)
...
pdf(paste0("output_final/map_ltbir_",nari,".pdf"))
...
for(cci in 1:llu){
  ...
  sa <- read.csv(paste0("output_final/",cni[cci],"_sa_",nari,"_infor_prior.csv"))[,-1]
  
  # 2014 level
  level2014 <- read.csv(paste0("output_final/level2014_",nari,"_infor_prior.csv"))[,-1]
  ...
  write.csv(s_new,paste0("output_final/s_all_",cni[cci],".csv"))
  ...
  # ggsave(paste0("output_final/eg_DS_age_",cni[cci],"_ltbis_when.pdf"), height = 10, width = 10)
  # 
  ...
  #   scale_fill_hue("clarity")
  # ggsave(paste0("output_final/eg_DR_age_",cni[cci],"_ltbir_when.pdf"), height = 10, width = 10)
  # 
  # 
  ...
  #   ggtitle("DS-TB") +
  #   scale_fill_hue("Year")
  # ggsave(paste0("output_final/DS_",cni[cci],"_ltbis_when.pdf"), height = 10, width = 20)
  # 
  ...
  #   ggtitle("MDR-TB") + 
  #   scale_fill_hue("Year")
  # ggsave(paste0("output_final/DR_",cni[cci],"_ltbir_when.pdf"), height = 10, width = 20)
  # 
  ...
  write.csv(meanv, paste0("output_final/",cni[cci],"_props_ltbi_when.csv"))
  ...
  ggsave(paste0("output_final/DR_mean_",cni[cci],"_ltbir_when.pdf"), height = 10, width = 20)
  
  ss_mean <- rbind(ss_mean, cbind(mean_both,cni[cci],ii))
  
  write.csv(paste0("output_final/",cni[cci],"_ss_mean_",nari,".csv"))[,-1]
